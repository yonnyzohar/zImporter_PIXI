...
const TimelineSprite_1 = require("./TimelineSprite");
class GameScene {
    constructor() {
        this.scene = null;
        this.valsToSetArr = [];
    ...
    spawn(tempName) {
        ...
        if (Object.keys(frames).length > 0) {
            mc = new TimelineSprite_1.TimelineSprite();
            this.createAsset(mc, baseNode);
            mc.setFrames(this.fixRotation(frames));
        }
        else {
            mc = new GameContainer_1.GameContainer();
            this.createAsset(mc, baseNode);
        }
    ...
    createAsset(mc, baseNode) {
        ...
        for (var i = 0; i < baseNode.children.length; i++) {
            ...
            if (type == "btn") {
                asset = new GameButton_1.GameButton();
            ...
            if (type == "asset") {
                ...
                if (Object.keys(frames).length > 0) {
                    asset = new TimelineSprite_1.TimelineSprite();
                    asset.setFrames(this.fixRotation(frames));
                }
                else {
                    asset = new GameContainer_1.GameContainer();
                }
                asset.name = child.instanceName;
                if (!asset.name) {
                    return;
